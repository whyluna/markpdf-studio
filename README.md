...
**文件即真相**：md 存纯文本源码、PDF 标注写回标准 PDF Annotation（系统预览可见）、AI 会话按文件存纯 JSON（`~/Library/Application Support/MarkPDF/ai-sessions.json`），不使用私有数据库锁死你的数据。核心功能单机离线、无账号、无遥测；AI 为可选能力——自带 API Key（存系统钥匙串），内容只在你显式发起时发往你自己配置的服务，首次使用有隐私告知。
...
| **AI 助手** | 右侧面板 `⌘⇧A` 多轮流式对话 · 三层上下文（选中文字 / 当前文档整篇或自动选节 / 工作区工具）· **Agent 检索**：AI 自主决定是否搜工作区、连续「搜索 → 看大纲 → 读章节」再作答，过程活动条可见 · 回答带 `[§章节]`/`[p.页码]` 锚点引用 · 回复可插入笔记 / 替换选区 / 存新笔记 / 转回链引用 · 会话按文档分线程、随工作区持久化 · 长对话自动压缩摘要不失忆 |
| **AI 配置** | 6 家 Provider（OpenAI / DeepSeek / Kimi / Qwen / Gemini / Claude）+ 自定义 baseURL · 一个 Provider 多模型、逐模型配置上下文窗口 · 翻译与助手独立选型 · Key 存钥匙串 · 连接测试 |
...
6. 笔记里 **⌘+点击** 回链 → 跳回 PDF 对应页并闪烁；右侧「引用」面板查看反向链接
