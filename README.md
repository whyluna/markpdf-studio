...
**文件即真相**：md 存纯文本源码、PDF 标注写回标准 PDF Annotation（系统预览可见）、AI 会话存工作区内 `.markpdf/` 纯 JSON，不使用私有数据库锁死你的数据。核心功能单机离线、无账号、无遥测；AI 为可选能力——自带 API Key（存系统钥匙串），内容只在你显式发起时发往你自己配置的服务，首次使用有隐私告知。
